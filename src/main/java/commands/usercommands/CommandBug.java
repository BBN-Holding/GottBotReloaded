...
public class CommandBug implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        if (args.length<=2) {
            event.getTextChannel().sendMessage(MessageHandler.getEmbed("usercommand.bug.help.title", "usercommand.bug.help.text", "normal", event)).queue();
        } else {
            ...
            event.getTextChannel().sendMessage(MessageHandler.getEmbed("util.sucess", "usercommand.bug.sucess.text", "normal", event)).queue();
        }
    }
