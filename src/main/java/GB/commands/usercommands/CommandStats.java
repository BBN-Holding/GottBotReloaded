...
public class CommandStats implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        ...
        event.getTextChannel().sendMessage(new EmbedBuilder().setTitle("Stats")
                .addField(MessageHandler.get("usercommands.stats.stats1",user,guild), new Handler().getMySQL().getfirst("stats",  "1", "command"),true)
                .addField(MessageHandler.get("usercommands.stats.stats2",user,guild), new Handler().getMySQL().getfirst("stats", "1", "message"),true)
