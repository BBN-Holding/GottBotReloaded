...
public class CommandVerification implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        if (event.getAuthor().getId()==event.getGuild().getOwner().getUser().getId() || event.getMember().hasPermission(Permission.MANAGE_SERVER) || Owner.get(event.getAuthor())) {
            ...
            if (args.length < 1) {
                event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("moderation.verification.title", "moderation.verification.text", "", "normal", event)).queue();
            } else {
                if (args[0].equalsIgnoreCase("setup")) {
                    if (args.length < 2) {
                        event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("moderation.verification.setup", "moderation.verification.setup2", "", "normal", event)).queue();
                    } else {
                        if (new Handler().getMySQL().get("server", "id", event.getGuild().getId(), "verification").equals("none")) {
                            if (event.getTextChannel().getMessageById(args[1]) != null) {
                                if (event.getGuild().getRoleById(args[2]) != null) {
                                    ...
                                    event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("util.sucess", "moderation.verification.set", zusatz, "sucess", event)).queue();
                                }
                            }
                        } else
                            event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("util.error", "moderation.verification.alreadyexist", "", "error", event)).queue();
                    }
                    ...
                    event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("util.sucess", "moderation.verification.reset", "", "normal", event)).queue();
                }
            }
        }
    }
