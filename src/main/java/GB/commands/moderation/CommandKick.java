    ...
    public void action(String[] args, MessageReceivedEvent event) {
        if (event.getAuthor().getId() == event.getGuild().getOwner().getUser().getId() || event.getMember().hasPermission(Permission.KICK_MEMBERS) || Owner.get(event.getAuthor())) {
            ...
            if (msg.getMentionedUsers().isEmpty()) {
                event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("moderation.kick.title", "moderation.kick.text", "", "normal", event)).queue();
            } else {
                ...
                if (!msg.getGuild().getSelfMember().canInteract(User)) {
                    ...
                    if (!User.getUser().isBot()) {
                        ...
                        channel.sendMessage(new Handler().getMessageHandler().getEmbed("moderation.kick.kick", "moderation.kick.user", event.getGuild().getName(), "sucess", event)).queue();
                    }
                    msg.getGuild().getController().kick(User).queue();
                    event.getTextChannel().sendMessage(new Handler().getMessageHandler().getEmbed("moderation.kick.kick", "moderation.kick.channel", User.getUser().getName(), "sucess", event)).queue();
                }
            }
        } else {
